...
const colors = [
  '#fb923c', '#f99039', '#f77e36', '#f66c33', '#f45a30',
  '#f2482d', '#f1362a', '#f02427', '#ef111f', '#dc2626',
]
...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    const stack = Composites.stack(0, 0, 6, 1, 0, 0, (x: number, y: number, index: number) => {
      ...
      return Bodies.rectangle(randomX, randomY, textWidth + 25, 50, {
        timeScale: 0.75,
        label: 'Box',
    ...
    const stackBalls = Composites.stack(0, 50, 5, 1, 0, 0, () => {
      ...
      return Bodies.circle(randomX, randomY, 25, {
        timeScale: 0.75,
        label: 'Box ',
        mass: 10,
        inverseMass: 1 / 10,
    ...
    Composite.add(world, [
      Bodies.rectangle(dimensions.width / 2, 0, dimensions.width, 1, { isStatic: true, restitution: 1, render: { fillStyle: '#ffffff' } }),
      Bodies.rectangle(dimensions.width / 2, dimensions.height, dimensions.width, 1, { isStatic: true, restitution: 1, render: { fillStyle: '#ffffff' } }),
      Bodies.rectangle(dimensions.width, dimensions.height / 2, 1, dimensions.height, { isStatic: true, restitution: 1, render: { fillStyle: '#ffffff' } }),
      Bodies.rectangle(0, dimensions.height / 2, 1, dimensions.height, { isStatic: true, restitution: 1, render: { fillStyle: '#ffffff' } }),
    ])
    ...
    Events.on(engine, 'collisionStart', (event) => {
      event.pairs.forEach(({ bodyA, bodyB, collision }) => {
        const box = bodyA.label.startsWith('Box') ? bodyA : bodyB.label.startsWith('Box') ? bodyB : null
        ...
        if (box && border) {
          ...
          if (contactX < boxCenterX && Math.abs(contactX - box.bounds.min.x) < 5) forceX = Matter.Common.random(0.06, 0.1)
          else if (contactX > boxCenterX && Math.abs(contactX - box.bounds.max.x) < 5) forceX = Matter.Common.random(-0.03, -0.1)
          if (contactY < boxCenterY && Math.abs(contactY - box.bounds.min.y) < 5) forceY = Matter.Common.random(0.04, 0.08)
          else if (contactY > boxCenterY && Math.abs(contactY - box.bounds.max.y) < 5) forceY = Matter.Common.random(-0.08, -0.12)
    ...
    Render.lookAt(render, { min: { x: 0, y: 0 }, max: { x: dimensions.width, y: dimensions.height } })
    ...
    return () => {
      ...
      Engine.clear(engine)
      if (currentScene) currentScene.innerHTML = ''
      ...
      <div className="fixed bottom-0 h-[700px] min-[350px]:h-[600px] sm:h-[500px] lg:h-[700px] w-full text-black p-4 lg:p-8 flex flex-col justify-end lg:grid grid-cols-2 gap-4">
        ...
        <div className="sm:grid-cols-[1fr,.4fr] grid gap-4 lg:gap-x-8 bg-orange-50 rounded-2xl px-[5vw] py-4 sm:py-8">
          ...
          <div className="flex flex-col">
            ...
            <button
              onClick={openModal}
              className="relative group block w-fit text-left"
            >
              Donation
              <div className="w-full absolute -bottom-[2px] left-0 h-0.5 flex justify-end group-hover:justify-start">
                <div className="group-hover:w-full bg-black h-full w-0 transition-all duration-300" />
          ...
          <div className="sm:col-span-2 sm:grid-cols-[1fr,.4fr] gap-4 sm:grid flex justify-between lg:items-end">
            <div className="flex items-center gap-4 lg:gap-6">
              <a href="http://www.linkedin.com/in/eyogi-gurukul-7a63a91a0" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors duration-300 flex items-center">
                <Linkedin className="w-5 h-5 lg:w-7 lg:h-7" />
              </a>
              <a href="http://www.twitter.com/@eyogigurukul" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors duration-300">
                <Twitter className="w-5 h-5 lg:w-7 lg:h-7" />
              </a>
              <a href="https://www.facebook.com/allfestivesireland#" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors duration-300">
                <Facebook className="w-5 h-5 lg:w-7 lg:h-7" />
              </a>
              <a href="https://www.youtube.com/channel/UCTytB2My0xSvNmtKBRIJnIg?" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors duration-300">
                <Youtube className="w-5 h-5 lg:w-7 lg:h-7" />
              </a>
            </div>
            <div className="gap-2">
              <p className="pl-1 lg:pl-2.5">&copy;2025</p>
              <a
                className="hover:text-black/80 text-sm flex gap-1 pl-1 lg:pl-2.5 flex-wrap transition-colors duration-300"
                href="https://www.it-wala.com"
                target="_blank"
                rel="noopener noreferrer"
              >
                Built by <span className="font-semibold">ITWALA</span>
              </a>
            </div>
          </div>
        </div>
