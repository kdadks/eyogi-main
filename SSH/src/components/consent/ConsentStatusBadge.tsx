...
const ConsentStatusBadge: React.FC<ConsentStatusBadgeProps> = ({
  ...
  return (
    <Component
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 rounded border font-medium whitespace-nowrap ${status.color} ${sizeClasses[size]} ${onClick ? 'cursor-pointer hover:opacity-80 transition-opacity' : ''} ${className}`}
